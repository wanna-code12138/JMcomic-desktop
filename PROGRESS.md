...
- 任务页新增"重试全部失败"一键批量重试（含已取消任务）：逐个重新入队，页面上方显示"已重新加入队列 n 个任务"；单个任务仍保留原有重试按钮
...
12. **顶部栏下载指示新增 hover 进度预览（新功能）**
    ...
    鼠标悬停在顶部栏"下载中 n"指示上，会弹出小浮层：逐任务显示漫画-章节、x/y 页与百分比进度条；"添加章节 x/y"阶段也会显示抓取进度。点击指示仍直达下载页。曾出现浮层被页面内容遮挡（标题栏 `backdrop-filter` 创建层叠上下文、浮层 z-index 无法逃逸），已把标题栏整体提到内容区之上修复。

13. **本地阅读器黑屏：本地章节字段名与阅读器不一致（已修复）**

    `resolveLocalChapterPages` 返回 `{index, url}`，而阅读器统一按 `page.imageUrl` 取图，导致本地模式下每张图的 src 都是空 → 黑屏。修复：本地章节统一返回 `imageUrl`（jmlocal:// 直读 URL），与网页模式 `PageData` 结构一致；回归测试断言了字段名。
...
- 2026-08-03：`fix/1.0.4-download-page-rows` 已合并（DB 行字段映射修复 + 一键重试全部失败）
