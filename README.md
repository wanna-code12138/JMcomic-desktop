...
│   ├── contentApi.ts        # 内容 IPC 处理器（首页/搜索/详情/章节图片）
...
│   ├── database.ts          # sql.js 封装（manga_cache / reading_history / downloads / favorites / auth）
│   ├── ipc.ts               # 通用 IPC（数据库、缓存、收藏、认证）
...
└── renderer/
    ...
    └── src/
        ...
        ├── assets/global.css
        ├── stores/
        │   └── appStore.ts  # zustand 全局状态（页面路由、阅读器状态、网络状态、主题）
        ├── pages/
        │   ├── HomePage.tsx         # 首页（推荐/最新/热门 Tab，一次请求三个 Tab 共享）
        │   ├── MangaDetailPage.tsx  # 漫画详情（封面、标签、章节列表、阅读/下载按钮）
        │   ├── ReaderPage.tsx       # 阅读器（滚动模式/单页模式、键盘翻页、jmimg:// 图片代理）
        │   ├── SearchPage.tsx       # 搜索页（⚠️ 仅 UI，未接入 API）
        │   ├── CategoriesPage.tsx   # 分类浏览（⚠️ 占位）
...
3. **SearchPage 未接入 API**：搜索框 UI 完成但输入关键词后不发起 `content:search` IPC 调用，也没有搜索结果展示。
4. **CategoriesPage 是占位**：显示"分类功能开发中"，未调用 `content:search`（siteAdapter 已有 `getCategory` 实现）。
5. **FavoritesPage 是占位**：登录按钮无点击处理，未接入 LoginDialog 和 `content:favorites`。
6. **DownloadsPage 是占位**：始终展示空状态，未调用 `download:list` IPC 查询数据库中的下载任务。
7. **SettingsPage 按钮无实际逻辑**：清空缓存、重新探测、代理输入、缓存大小滑块都没有绑定 handler。
8. **深色模式切换未生效**：TitleBar 传递了 `darkMode` prop 但 App 组件没有根据它切换 Fluent UI 主题（需要 `<FluentProvider theme={...}>` 包裹）。

### 🟢 改进建议

9. **阅读器无磁盘缓存集成**：ReaderPage 直接通过 `jmimg://` 代理加载图片，没有预取到磁盘缓存。已浏览的图片每次打开都需要重新从 CDN 加载（虽然 `jmimg://` 设置了 `Cache-Control: max-age=86400`，但这是 HTTP 缓存而非应用层缓存）。
10. **无阅读进度记忆**：`reading_history` 表已建好但 ReaderPage 不会保存/恢复上次阅读位置。
11. **无章节预加载**：阅读当前章时不预加载下一章的首几张图，翻章时有明显等待。
12. **首页无分页/无限滚动**：只提取第一页内容（~24 张卡片），无法浏览更多。
13. **下载无 CBZ/ZIP 导出**：下载的图片以文件夹形式保存，不支持打包为漫画阅读器通用格式。
14. **内容提取无重试**：scraperWindow 导航失败时直接抛错，没有自动重试或切换备用域名。
15. **`NavigationView.tsx` 组件未被使用**：App.tsx 里导航是直接内联写的，`NavigationView` 组件是独立封装但未引用（冗余代码）。
...
2. SearchPage 接入 `content:search` IPC，显示搜索结果卡片
3. FavoritesPage 集成登录对话框和收藏列表
4. DownloadsPage 接入 `download:list` 显示下载进度
5. SettingsPage 绑定实际功能（清缓存、重探测、代理配置）
6. 深色模式生效（FluentProvider 主题切换）
7. 阅读进度自动保存/恢复
